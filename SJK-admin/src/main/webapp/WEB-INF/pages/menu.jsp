...
function operateFormatter(value, row, index) {
    return ['<button type="button" class=" btn btn-info" onclick="getValue('+row.menuId+')">修改</button>',
        '&nbsp;&nbsp;&nbsp;<button class=" btn btn-danger" type="button" onclick="delMenu('+row.menuId+')">删除</button>'
        ].join('');
}
...
function getValue(id){
	$.ajax({
		...
		success:function(data){
			$("#menu_id1").val(id);
			$("#menu_name1").val(data.menu.menuName);
			...
			$.each(data.list,function(index,items){
				if(data.menu.parentId==items.menu_id){
					$("#parentId1").append("<option selected value="+items.menuId+">"+items.menuName+"</option>");
				}else{
					$("#parentId1").append("<option value="+items.menuId+">"+items.menuName+"</option>");
				}
...
function checkMenu1(){
	...
	if(x!=""){
		$.ajax({
			url:'${path }/menu/checkName.action',
			dataType:'json',
			type:'post',
			data:{
				menuName:x,
